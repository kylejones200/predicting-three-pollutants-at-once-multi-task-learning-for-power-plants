...
emissions_df = pd.DataFrame({'log_co2': np.log1p(co2), 'log_nox': np.log1p(nox), 'log_so2': np.log1p(so2)}).dropna()
corr = emissions_df.corr()
logger.info('Emissions Correlations:')
logger.info(corr)
sns.heatmap(corr, annot=True, cmap='RdYlGn', center=0, square=True, linewidths=2, cbar_kws={'shrink': 0.8})
...
def build_mtl_model(input_dim, architecture='hard_sharing'):
    ...
    model = keras.Model(inputs=inputs, outputs=[co2_output, nox_output, so2_output], name='mtl_emissions_predictor')
    return model
model = build_mtl_model(input_dim=10)
model.summary()
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import StandardScaler
feature_cols = ['Plant nameplate capacity (MW)', 'Plant annual net generation (MWh)', 'Plant annual heat input (MMBtu)', 'Plant state abbreviation', 'Plant primary fuel category']
...
X_encoded = pd.get_dummies(X[['Plant state abbreviation', 'Plant primary fuel category']], drop_first=True)
X_features = pd.concat([X[['capacity_mw', 'generation_mwh', 'heat_input_mmbtu']], X_encoded], axis=1)
...
X_train, X_test, y_co2_train, y_co2_test = train_test_split(X_features, y_co2, test_size=0.2, random_state=42)
...
mtl_model.compile(optimizer=keras.optimizers.Adam(learning_rate=0.001), loss={'co2_output': 'mse', 'nox_output': 'mse', 'so2_output': 'mse'}, metrics={'co2_output': ['mae', 'mse'], 'nox_output': ['mae', 'mse'], 'so2_output': ['mae', 'mse']})
history = mtl_model.fit(X_train_scaled, {'co2_output': y_co2_train, 'nox_output': y_nox_train, 'so2_output': y_so2_train}, validation_split=0.2, epochs=50, batch_size=64, callbacks=[keras.callbacks.EarlyStopping(patience=10, restore_best_weights=True), keras.callbacks.ReduceLROnPlateau(patience=5, factor=0.5)], verbose=1)
results = mtl_model.evaluate(X_test_scaled, {'co2_output': y_co2_test, 'nox_output': y_nox_test, 'so2_output': y_so2_test})
...
co2_model.fit(X_train_scaled, y_co2_train, validation_split=0.2, epochs=50, batch_size=64, callbacks=[keras.callbacks.EarlyStopping(patience=10)], verbose=0)
co2_baseline_mae = co2_model.evaluate(X_test_scaled, y_co2_test, verbose=0)[1]
nox_model = build_single_task_model(input_dim)
nox_model.fit(X_train_scaled, y_nox_train, validation_split=0.2, epochs=50, batch_size=64, callbacks=[keras.callbacks.EarlyStopping(patience=10)], verbose=0)
nox_baseline_mae = nox_model.evaluate(X_test_scaled, y_nox_test, verbose=0)[1]
so2_model = build_single_task_model(input_dim)
so2_model.fit(X_train_scaled, y_so2_train, validation_split=0.2, epochs=50, batch_size=64, callbacks=[keras.callbacks.EarlyStopping(patience=10)], verbose=0)
...
comparison = pd.DataFrame({'Task': ['CO₂', 'NOx', 'SO₂'], 'Single-Task MAE': [co2_baseline_mae, nox_baseline_mae, so2_baseline_mae], 'MTL MAE': [results[4], results[7], results[10]]})
comparison['Improvement %'] = (comparison['Single-Task MAE'] - comparison['MTL MAE']) / comparison['Single-Task MAE'] * 100
logger.info('\nMTL vs Single-Task Comparison:')
logger.info(comparison.to_string(index=False))
fig, ax = plt.subplots(figsize=(10, 6))
x = np.arange(len(comparison))
width = 0.35
ax.bar(x - width / 2, comparison['Single-Task MAE'], width, label='Single-Task', color='#e74c3c', alpha=0.8)
ax.bar(x + width / 2, comparison['MTL MAE'], width, label='Multi-Task', color='#2ecc71', alpha=0.8)
...
for i, row in comparison.iterrows():
    ax.text(i, max(row['Single-Task MAE'], row['MTL MAE']) + 0.02, f"+{row['Improvement %']:.1f}%", ha='center', fontsize=11, fontweight='bold', color='green')
plt.tight_layout()
plt.savefig('mtl_vs_single_task.png', dpi=150)
mtl_model_weighted = build_mtl_model(input_dim)
mtl_model_weighted.compile(optimizer=keras.optimizers.Adam(learning_rate=0.001), loss={'co2_output': 'mse', 'nox_output': 'mse', 'so2_output': 'mse'}, loss_weights={'co2_output': 2.0, 'nox_output': 1.0, 'so2_output': 1.0}, metrics={'co2_output': ['mae'], 'nox_output': ['mae'], 'so2_output': ['mae']})
history_weighted = mtl_model_weighted.fit(X_train_scaled, {'co2_output': y_co2_train, 'nox_output': y_nox_train, 'so2_output': y_so2_train}, validation_split=0.2, epochs=50, batch_size=64, callbacks=[keras.callbacks.EarlyStopping(patience=10)], verbose=0)
results_weighted = mtl_model_weighted.evaluate(X_test_scaled, {'co2_output': y_co2_test, 'nox_output': y_nox_test, 'so2_output': y_so2_test}, verbose=0)
...
def build_soft_sharing_model(input_dim, n_tasks=3):
    ...
    for i in range(n_tasks):
        net = layers.Dense(128, activation='relu', kernel_regularizer=keras.regularizers.l2(0.01))(inputs)
        net = layers.Dense(64, activation='relu', kernel_regularizer=keras.regularizers.l2(0.01))(net)
        net = layers.Dense(32, activation='relu')(net)
        pd.concat([task_networks, net])
...
metrics = {'co2_output': ['mae', 'mse'], 'nox_output': ['mae', 'mse'], 'so2_output': ['mae', 'mse']}
(X_train_scaled,)
({'co2_output': y_co2_train, 'nox_output': y_nox_train, 'so2_output': y_so2_train},)
validation_split = (0.2,)
epochs = (50,)
batch_size = (64,)
callbacks = ([keras.callbacks.EarlyStopping(patience=10, restore_best_weights=True), keras.callbacks.ReduceLROnPlateau(patience=5, factor=0.5)],)
verbose = 1
'Single-task model for comparison'
model = keras.Sequential([keras.Input(shape=(input_dim,)), layers.Dense(128, activation='relu'), layers.BatchNormalization(), layers.Dropout(0.3), layers.Dense(64, activation='relu'), layers.BatchNormalization(), layers.Dropout(0.3), layers.Dense(32, activation='relu'), layers.BatchNormalization(), layers.Dense(16, activation='relu'), layers.Dense(1)])
model.compile(optimizer=keras.optimizers.Adam(learning_rate=0.001), loss='mse', metrics=['mae', 'mse'])
